...
本文档系统介绍了 Generative Agents 项目的核心技术架构设计、三大核心认知机制、已完成的生存演化重构与性能优化，以及真实的 `step` 循环运算链路。附录包含尚未实现的远景规划。
...
1.  **铁律 1：认知大脑（LLM）负责“随机应变”**  
    智能体不应遵循死板的预设行为脚本。系统在每一步将足够的环境状态、生理数值及记忆上下文组装并塞给大模型，由大模型这个“认知大脑”给出具体方案 and 行动（自主决策）。
2.  **铁律 2：硬编码仅负责“物理底座”**  
...
4. [真实 Step 循环运算链路](#4-真实-step-循环运算链路)
   - [4.1 Step 主循环执行顺序](#41-step-主循环执行顺序)
   - [4.2 环境同步与前后端锁步](#42-环境同步与前后端锁步)
   - [4.3 世界状态同步与代谢更新](#43-世界状态同步与代谢更新)
   - [4.4 Persona.move() 决策分支流程](#44-personamove-决策分支流程)
   - [4.5 认知核心组件分析](#45-认知核心组件分析)
   - [4.6 存档与落盘关系](#46-存档与落盘关系)
...
> 2. **Associative Memory (联想记忆流)**：以 `ConceptNode` 节点形式存储事件、想法 and 对话。
...
* **改造前**：无论智能体处于什么状态，每一步都必须完整运行 Perceive → Retrieve → Plan → Reflect 的完整流程，造成了大量重复 of LLM 请求。
...
### 4.1 Step 主循环执行顺序
后端主循环入口是 [reverie.py](file:///Users/gun/mygame/generative_agents/reverie/backend_server/reverie.py) 中的 `ReverieServer.start_server()`。在每个 Step（即 10 秒游戏 Tick）中，系统执行以下核心流程：

1. **环境状态同步**：从 Django 或本地 `storage/` 目录拉取当前 Step 前端上报的环境帧数据。
2. **待处理动作注入**：加载并注入 Creator 或用户下达的即时操作指令。
3. **清除上轮对象事件**：将上轮因动作执行产生的临时家具状态/事件进行恢复和清理。
4. **回写位置与阻挡**：将上一步所有 Persona 最终坐标同步至后端二维网格地图 `Maze`。
5. **代谢与生理更新**：针对所有活体角色计算生理值扣减（Stamina 疲劳、Satiety 饱食衰减）。
6. **并发执行 `move()` 认知**：利用 `ThreadPoolExecutor` 并发驱动每一个 Persona 脑部推演。
7. **汇总并分发 movement**：合并所有小人新产生的坐标和发言气泡，写入本地 JSON 并发送回 Django 以供前台渲染。
8. **时间推移与增量保存**：递增 step 序号，保存角色状态。

```
ReverieServer.start_server()
  -> 1. 环境同步 / 2. 用户动作注入 / 3. 清理残存事件
  -> 4. 把位置同步回后端 maze
  -> 5. 统一代谢值更新 (Stamina/Satiety 衰减)
  -> 6. 并发 persona.move() 认知流水线
  -> 7. 汇总 movement 并写入磁盘/发送 Django
  -> 8. step + 1, 时间推进, 增量序列化保存
```

### 4.2 环境同步与前后端锁步
为了防止后台引擎推演过快，而前台 Phaser 播放动画过慢，系统引入了**前后端步进锁同步机制（Lockstep Synchronization）**：
* **心跳心脉**：若检测到 `temp_storage/frontend_active_<sim_code>.json` 的前台心跳在 10s 内更新，说明用户在网页观看。
* **锁步挂起**：此时，后端在每一步开始前会轮询检查当前 Step 的 Django 环境帧。若前台未完成上轮动画上报最新帧，后端会自动以 0.5s 间隔挂起等待。
* **解耦退避**：当心跳超时（如关闭网页），挂起打断，后端自动以最高 0.1s/步 的离线解耦速率全速推演。

### 4.3 世界状态同步与代谢更新
在正式触发智能体大脑（`move()`）前，物理引擎层进行代谢扣减，保证智能体在本步看到的是最新已衰减的身体指标：
* 行动或奔跑时，精力衰减加速；睡眠与休息能实现 Stamina 大幅充能；
* 饱食度归 0 时，扣减生命值（`health -= 2.0`），直到在 ReAct 生存打断中获取到食物并执行 `Consume` 恢复。

### 4.4 Persona.move() 决策分支流程
`Persona.move()` 是单个智能体 Step 内的“指挥塔”，它负责把小人推上三条管道之一：

1. **死亡短路 (Death Freeze)**：若 `health <= 0`，智能体逻辑冻结，不走任何后续感知与动作。
2. **生存反应打断 (Survival React Interruption)**：若 `satiety < 30.0` 或 `stamina < 20.0`，系统判定面临生理危机。此时强制清空 `planned_path` 和当前动作，进入完整认知链以调用 `decide_survival_action` 重写求生日程。
3. **快速路径执行 (Fast Path)**：若没有生理危机、当天并非刚过 0 点，且 `planned_path`（A* 行走路径）非空，小人将只运行一次极其轻量的社交距离扫描，然后直接进入 `execute()` 输出下个格点坐标，**跳过 LLM 认知大脑，大幅节约 Token 消耗**。
4. **完整认知链路 (Full Cognitive Pipeline)**：在不符合以上条件时（例如动作刚结束或新一天起床），进入完整的大脑认知周期：`Perceive -> Retrieve -> Plan -> Reflect -> Execute`。

### 4.5 认知核心组件分析
在完整认知链路中，核心文件的分工如下：
* **`perceive.py` (感知)**：扫描视野内 tile 上的物件和事件，将新事件转化为 `ConceptNode`。若检测到的事件重要性累计超过 150，将扣减反思计数器并触发 Reflect。
* **`retrieve.py` (检索)**：常规 step 规划只做“轻检索”（只拿跟当前事件相关的 thoughts/events），不计算 embedding，速度较快；反思与社交时则走“重检索”（综合 Recency、Relevance 和 Importance 评分）。
* **`plan.py` (规划)**：
  * 主口 `decide_demand_action()` 是一段压缩式决策层。它先收集状态和生理规则，发给 LLM 获取决策，接着在程序侧对结果进行纠偏（例如把无食物时的 `Consume` 换成 `Gather`），最后通过 A* 寻路和地址映射写入 `scratch`。
  * `lets_talk()` 用于 NPC 社交前置校验，若通过 LLM 判定，则为双方同时注入 10 分钟的聊天占位日程。
* **`reflect.py` (反思)**：定期运行。LLM 总结最近 100 条记忆片段，并提炼出 5 条高层级 Thoughts 存入记忆。
* **`execute.py` (执行守卫)**：检查 `planned_path`，逐格向 movement 移进。一旦路径制造完毕，根据 `skill_id` 调度具体技能包（如 `ChatSkillPack`、`GatherSkillPack` 等）进行物理与属性结算。若发生寻路失败，则记录 `navigation_failure` 并挂起重决策。

### 4.6 存档与落盘关系
* **`environment/{step}.json`**：Django 视图 `process_environment()` 收集的前台小镇各 tile 和角色最终坐标。
* **`movement/{step}.json`**：后端推演出的动作/位置增量数据，供前台拉取播放。
* **增量序列化**：每隔一定步数，主模拟循环会自动对 `personas/` 下的角色关联记忆 `nodes.json` 和 `scratch.json` 进行序列化保存，以在崩溃或手动 `save` 时完美恢复状态。

---

## 附录：尚未实现的远景规划

以下功能来自早期的生存演化沙盒重构提案，尚未实现，作为后续迭代方向保留：

### A.1 结构化知识库与书籍系统
1. **结构化个人知识 (Personal Knowledge)**：以 KV 结构与语义向量双重存储，记录生存常识和配方知识。智能体可在决策时检索知识库。
2. **书籍阅读与文档系统 (Document Library)**：智能体可通过 `Read` 工具阅读书架上的外部文档，获得的配方写入个人知识库；亦可执行 `Write` 工具撰写"生存手记"遗留在环境中，供其他智能体阅读学习，实现跨个体知识传递。

### A.2 升级式技能熟练度与天赋系统
1. **职业技能 XP 升级**：等级（Level 1-10）与经验值体系。农业 (Farming) 影响采集产出量，烹饪 (Cooking) 影响合成食物的能量转换效率。
2. **天赋/特殊特质 (Perks)**：升级或反思后概率习得的被动特质（如御寒体质：雪天精力消耗率降低 30%）。
3. **对人设的动态反向修改**：技能升级后，ISS 特质自动拼接职业属性附言。

### A.3 自适应进化与社会性传承
1. **个体认知适应性演进**：在反思阶段复盘近期求生状况，成功策略沉淀为"程序性知识"，静态 Trait 特质随环境恶劣程度动态调整。
2. **知识共享与技能传授**：高等级智能体在社交中自发向低等级智能体传授配方或采摘路线。
3. **原始经济体系演化**：职业技能分工导致资源置换意图，通过对话协商确定兑换价格，逐步演化出以物易物的经济体系。

### A.4 环境动态要素扩展
1. **资源消耗与自然再生**：每次 Gather 采集扣减坐标处储量，储量随时间步推移自动再生。
2. **环境压力模拟 (Hazards)**：昼夜交替、天气系统（大雨、雪天、烈日），恶劣环境加剧精力和饱食度流失。
